...
## Вопросы владельцу
- `EXECUTION-BACKEND.md` лежит в корне репозитория, но раздел «Запуск сессии» в нём ссылается на `docs/EXECUTION-BACKEND.md`. Переместить файл в `docs/` или поправить путь в тексте?
